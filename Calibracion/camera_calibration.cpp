...
class Settings
{
 public:
	 ...
	 void interprate ()
	 {
			...
			{
				 cerr << "Invalid Board size: " << boardSize.
						width << " " << boardSize.height << endl;
				 goodInput = false;
			}
			if (squareSize <= 10e-6)
			{
			...
			else
			{
				 ...
				 else
				 {
						if (readStringList (input, imageList))
						{
							 ...
							 nrFrames =
									(nrFrames <
									 (int) imageList.size ())? nrFrames : (int) imageList.
									size ();
						}
						else
	 ...
	 for (int i = 0;; ++i)
	 {
			...
			if (mode == CAPTURING && imagePoints.size () >= (unsigned) s.nrFrames)
			{
				 if (runCalibrationAndSave
						 (s, imageSize, cameraMatrix, distCoeffs, imagePoints))
						mode = CALIBRATED;
				 else
						mode = DETECTION;
			}
			...
			case Settings::ASYMMETRIC_CIRCLES_GRID:
				 found =
						findCirclesGrid (view, s.boardSize, pointBuf,
			...
			char key = (char) waitKeyEx (s.inputCapture.isOpened ()? 50 : s.delay);
	 ...
	 case Settings::CIRCLES_GRID:
			for (int i = 0; i < boardSize.height; ++i)
				 for (int j = 0; j < boardSize.width; ++j)
						corners.
							 push_back (Point3f
													(float (j * squareSize), float (i * squareSize),
													 0));
			break;
															...
															double totalAvgErr)
{
	 ...
	 if (!rvecs.empty () || !reprojErrs.empty ())
			fs << "nrOfFrames" << (int) std::max (rvecs.size (),
																						reprojErrs.size ());
