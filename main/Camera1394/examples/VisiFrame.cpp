/*  
 	VisiFrame.cpp - an example that captures images from one camera,
	and display it on a window.
...
public:
    MyFrame(const uint32_t CamNum=0) : wxFrame((wxFrame *)NULL, -1,  wxT("CVgui"), wxPoint(50,50), wxSize(640,480)), Camera1394 (CamNum)
    {
...
bool MyApp::OnInit()
{
    frame = new MyFrame();
	 ...
	 frame->Print_Cameras_List();

    frame->Print_Camera_Info();

    frame->Setup (5, DC1394_VIDEO_MODE_640x480_MONO8, DC1394_ISO_SPEED_800, DC1394_FRAMERATE_30, 5, RGB8);

    frame->Set_Filename_Base("Camera_Zero");
   
    frame->Set_MaxFileCounter(10000);

 	 frame->Set_FileSaveType(FST_ALL);

    frame->Set_CaptureInterval (0.5);
